...
目标：在账号和对局落库基础上，建立可持续使用的积分、段位和排行榜系统。
...
1. 确定积分规则和段位配置。
2. 调整 Prisma 数据模型，加入积分和积分流水。
3. 在对局落库后计算并保存积分变化。
4. 新增个人排名和排行榜接口。
5. 客户端接入个人积分、段位、排名和排行榜页面。
6. 补充服务端测试，覆盖重复结算和积分上下限。
